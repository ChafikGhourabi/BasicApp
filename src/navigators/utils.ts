...
export function useBackButtonHandler(canExit: (routeName: string) => boolean) {
  ...
  const canExitRef = useRef((routeName: string) =>
    Config.exitRoutes.includes(routeName),
  );
...
export function useNavigationPersistence(
  ...
  const onNavigationStateChange: NavigationProps['onStateChange'] = state => {
    ...
    const currentRouteName = getActiveRouteName(
      state as ReturnType<typeof navigationRef.getRootState>,
    );
  ...
  return {
    ...
    restoreState,
    isNavigationStateRestored: isRestored,
