...
import { clear, loadAll } from '@/utils/storage';
import { resetRoot, goBack, navigate } from '@/navigators';
...
Reactotron.onCustomCommand({
  title: 'Load All Storage',
  description: 'Loads all stored values in Storage',
  command: 'loadStorage',
  handler: async () => {
    const storedValues = await loadAll();
    Reactotron.log(storedValues);
  },
});

Reactotron.onCustomCommand({
  title: 'Reset Root Storage',
  description: 'Resets the storage',
  command: 'resetStorage',
  handler: () => {
    Reactotron.log('resetting storage');
    clear();
  },
});
